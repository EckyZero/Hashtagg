...
namespace iOS
{
	public class DoctorRecommendationAnnotation : RecommendationAnnotation
	{
		public DoctorRecommendationAnnotation (MapMarkerViewModel vm) : base(vm)
		{
		...
		private string GetImagePathNoDir()
		{
			...
			if(_viewModel.MarkerType == MapMarkerType.PreferredLocation){
				...
				if(_viewModel.Rating == MapMarkerState.Negative){
					imageFilename = string.Format ("yd_orange{0}.png", _viewModel.Rank);
				}
				else if(_viewModel.Rating == MapMarkerState.Neutral){
					imageFilename = string.Format ("yd_gray{0}.png", _viewModel.Rank);
				}
				else if(_viewModel.Rating == MapMarkerState.Positive){
					imageFilename = string.Format ("yd_green{0}.png", _viewModel.Rank);
				}
